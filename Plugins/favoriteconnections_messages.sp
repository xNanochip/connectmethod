...
public OnPluginStart()
{
	...
	hBroadcastMode = CreateConVar("favoriteconnections_messages_broadcastmode", "1", "How should we broadcast to all players on the server about the player joining through favorites? 0 = Do nothing 1 = Print to the chat 2 = Print to the center of the screen 3 = Both print to chat and center of the screen. (1 is the default)", FCVAR_NOTIFY);
	hNotifyMode = CreateConVar("favoriteconnections_messages_notifymode", "1", "How should we notify the player who joined through favorites? 0 = Do nothing 1 = Print to the chat 2 = Print to the center of the screen 3 = Both print to chat and center of the screen. (1 is the default)", FCVAR_NOTIFY);
	hBroadcastChat = CreateConVar("favoriteconnections_messages_broadcastchat", "{green}%PLAYER_NAME% {orange}joined the server through his/her {blue}favorites{orange}!", "This is the chat text that is sent to all players on the server about the player joining through favorites (Supports colors).", FCVAR_NOTIFY);
	hBroadcastCenter = CreateConVar("favoriteconnections_messages_broadcastcenter", "%PLAYER_NAME% joined the server through his/her favorites!", "This is the center text that is sent to all playeres on the server about the player joining through favorites (Doesn't support colors).", FCVAR_NOTIFY);
	hNotifyChat = CreateConVar("favoriteconnections_messages_notifychat", "{orange}Thank you, {green}%PLAYER_NAME%{orange}, for adding us to your favorites!", "This is the chat text that is sent to only the player when he/she joins the game via favorites (Supports colors).", FCVAR_NOTIFY);
	hNotifyCenter = CreateConVar("favoriteconnections_messages_notifycenter", "Thank you, %PLAYER_NAME%, for adding us to your favorites!", "This is the center text that is sent to only the player when he/she joins the game via favorites (Doesn't support colors).", FCVAR_NOTIFY);
